    ...
    companion object {
        ...
        private fun getConnectionType(context: Context): String? {
            return try {
                ...
                for (item: Network in networks) {
                    ...
                    if (item == n) {
                        connType = when {
                            result?.hasTransport(NetworkCapabilities.TRANSPORT_CELLULAR) == true -> "mobile"
                            result?.hasTransport(NetworkCapabilities.TRANSPORT_WIFI) == true -> "wifi"
                            else -> connType
                        }
                        break
                    }
        ...
        private fun setCarrierInfo(deviceInfo: DeviceInfo, context: Context) {
            try {
                ...
                deviceInfo.countryCode = tm?.networkCountryIso?.uppercase(Locale.getDefault())
